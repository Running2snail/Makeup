...
//varying highp vec2 textureCoordinate;   // 已在定点着色器中保存  ✔
varying highp vec2 blurCoord1s[14];     // 已在顶点着色器中处理  ✔
...
void main(){
    ...
    //通过绿色通道来磨皮
    //取得当前点颜色的绿色通道
    central = texture2D(videoframe, coordinate).g;
    ...
    for (int i = 0; i < 6; i++) {
        //采样点的绿色通道
        sampleColor = texture2D(videoframe, blurCoord1s[i]).g;
        //采样点和计算点的颜色差
    ...
    //取得当前点的颜色
    centralColor = texture2D(videoframe, coordinate ).rgb;
    //采样值
